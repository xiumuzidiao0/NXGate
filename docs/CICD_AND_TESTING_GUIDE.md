...
以下 5 个文件中的版本号必须**绝对相同**（例如 `2.5.7` 或子版本 `2.5.7.1`）：

| 序号 | 文件路径 | 版本定义形态 | 说明 |
| :--- | :--- | :--- | :--- |
| 1 | `VERSION` | `2.5.7` | 根目录纯文本标识，供安装脚本与自动化流水线直接读取 |
| 2 | `pkg/config/version.go` | `const Version = "2.5.7"` | 编译进二进制内部的版本常量，供 `--version` 输出 |
| 3 | `install.sh` | `DEFAULT_VERSION="2.5.7"` | 一键脚本内置的默认版本及 fallback 下载路径 |
| 4 | `web/dist/index.html` | 侧栏/顶栏 badge: `v2.5.7` | 前端页面直接渲染的静态徽章版本 |
| 5 | `android/app/build.gradle.kts` | `versionName = "2.5.7"` | Android 原生客户端版本名定义 |

### 2. 版本号命名与演进规范 (SemVer + 4位微补丁子版本)

- **常规特性 / 补丁发布 (3位 SemVer)**：`2.5.8`、`2.6.0`
  - 适用于常规功能迭代、多模块架构重构或累计的大量更新。
- **微小改动 / 紧急 Hotfix (4位微版本)**：`2.5.7.1`、`2.5.7.2`
  - **核心设计意图**：Web 控制台的在线更新检查器与运维脚本 `nx update` 是通过 `CompareVersions(latest, current) > 0` 来决定是否向用户弹出“发现新版本”更新提示横幅的；
  - 若进行较小改动（如配置微调、单个协议细节修复、依赖精简等）且不希望递增整个 Patch 位时，**强烈推荐使用 4 位子版本号**（如 `v2.5.7.1`），确保线上客户端能够立即无缝检测到新版本并触发热更新。
...
#### 一键同步所有版本：
在准备发布新版本时，**仅需执行一行命令**即可将全工程 5 处版本全部更新：
```bash
./scripts/check-version.sh --set 2.5.7.1
```
